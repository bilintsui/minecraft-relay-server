...
static bool short_test_reload_late_release(const char *binary, const char *directory) {
	...
	short_fixture fixture = { .notify_fd = -1, .listener = -1 };
	int client_fd = -1;
	CHECK(short_test_fixture_start(&fixture, binary, directory, "reload-late", "127.0.0.1", 9) == 0,
	...
	while (nanosleep(&delay, &delay) == -1) {
		CHECK(errno == EINTR, "could not wait for the late-reload client to enter initial state");
	}
	CHECK(short_test_config_write(&fixture, "127.0.0.1", 9) == 0, "could not rewrite configuration for first reload");
	...
	CHECK(short_test_config_write(&fixture, "127.0.0.1", 9) == 0, "could not rewrite configuration for deferred reload");
	...
	CHECK(notification_size > 0 && strncmp(notification, "RELOADING=1\nMONOTONIC_USEC=", strlen("RELOADING=1\nMONOTONIC_USEC=")) == 0,
		"deferred late-reload notification was missing after late route release");
	notification_size = short_test_fixture_notification(&fixture, notification, sizeof(notification));
...
int main(int argc, char **argv) {
	...
	test_result = short_test_admission(argv[1], temp_directory) && short_test_deadline(argv[1], temp_directory)
		...
		&& short_test_reload_late_release(argv[1], temp_directory)
