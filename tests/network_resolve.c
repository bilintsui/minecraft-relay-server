...
static bool resolver_test_dual(void) {
	...
	CHECK(result.err == NET_OK && net_address_equal(&result, AF_INET, mock_address_first), "primary NSS result was not selected");
	...
	CHECK(result.err == NET_OK && net_address_equal(&result, AF_INET, mock_address_first), "alternate NSS result was not selected");
...
static bool resolver_test_literals(void) {
	...
	CHECK(result.err == NET_OK && net_address_equal(&result, AF_INET, address_v4), "IPv4 literal was not resolved directly");
	CHECK(mock_call_count == 0, "IPv4 literal reached NSS");

	result = net_resolve_dual("2001:db8::40", AF_INET6, false);
	CHECK(result.err == NET_OK && net_address_equal(&result, AF_INET6, address_v6), "IPv6 literal was not resolved directly");
	CHECK(mock_call_count == 0, "IPv6 literal reached NSS");

	resolver_mock_reset(AF_UNSPEC, NULL, NULL);
	result = net_resolve_dual("192.0.2.40", AF_INET6, true);
	CHECK(result.err == NET_OK && net_address_equal(&result, AF_INET, address_v4), "wrong-family literal did not use dual fallback");
...
int main(void) {
	if (!resolver_test_address_parse() || !resolver_test_dual() || !resolver_test_invalid() || !resolver_test_literals()) {
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}
