...
static bool resolver_test_address_parse(void) {
	...
	CHECK(result.err == 0 && net_address_equal(&result, AF_INET, address_v4), "IPv4 literal was not parsed");
	result = net_addr_parse("2001:db8::40");
	CHECK(result.err == 0 && net_address_equal(&result, AF_INET6, address_v6), "IPv6 literal was not parsed");
	result = net_addr_parse("0.0.0.0");
	CHECK(result.err == 0 && result.family == AF_INET && result.addr.v4 == INADDR_ANY, "IPv4 wildcard was not parsed");
	result = net_addr_parse("::");
	CHECK(result.err == 0 && result.family == AF_INET6 && memcmp(result.addr.v6, &in6addr_any, sizeof(result.addr.v6)) == 0, "IPv6 wildcard was not parsed");
