...
p_proxy protocol_proxy_read(const void *src, size_t n)
{
	...
	sscanf(src + 11, "%s %s %hu %hu\r\n", (char *)&srcaddrp,
	       (char *)&dstaddrp, &(result.srcport), &(result.dstport));
...
size_t protocol_proxy_write(void *dst, p_proxy src)
{
	...
	case AF_INET:
		return sprintf(dst, "PROXY TCP4 %s %s %hu %hu\r\n",
			       (char *)&srcaddrp, (char *)&dstaddrp,
			       src.srcport, src.dstport);
	case AF_INET6:
...
size_t protocol_proxy_write_plain(void *dst, sa_family_t family,
				  net_addr srcaddr, net_addr dstaddr,
