/*
 * connection/setup.c: Functions for initial connection setup
 *
...
static connsetup_status connsetup_prepared_run(int socket_in, int *socket_out, net_addrbundle addrinfo_in, const uint8_t *inbound, size_t inbound_size, const connsetup_snapshot *snapshot) {
	...
	switch (protocol) {
		...
		case PVER_LEGACYL3:
		case PVER_LEGACYL4:
			return connsetup_handle_legacy_login(socket_in, socket_out, addrinfo_in, inbound, inbound_size, snapshot);
		case PVER_MODERN1:
		case PVER_MODERN2:
			if (intent == CLIENT_INTENT_LOGIN || intent == CLIENT_INTENT_TRANSFER) {
				return connsetup_handle_modern_handshake(socket_in, socket_out, addrinfo_in, inbound, inbound_size, snapshot);
			}
			break;
		case PVER_ORIGPRO:
			close(socket_in);
...
connsetup_status connsetup_prepared(int socket_in, int *socket_out, const connsetup_snapshot *snapshot, net_addrbundle addrinfo_in, const uint8_t *inbound, size_t inbound_size) {
	...
	return connsetup_prepared_run(socket_in, socket_out, addrinfo_in, inbound, inbound_size, snapshot);
}
