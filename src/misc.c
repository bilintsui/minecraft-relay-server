	     ...
	     short netpriority_enabled)
{
	unsigned char inbound[BUFSIZ], outbound[BUFSIZ], rewrited[BUFSIZ],
	    pheader[PROTOPROXY_PACKETMAXLEN + 1];
	...
	case 0xFE:
		if (packlen_inbound > 2) {
			while (packlen_inbound < 0x20) {
				packlen_inbound_append =
				    packlen_inbound + recv(socket_in,
							   inbound +
							   packlen_inbound,
							   BUFSIZ -
							   packlen_inbound, 0);
				if (packlen_inbound_append == 0) {
					mksysmsg(0, logfile, runmode,
						 conf_in->log.level, 1,
						 "src: %s:%d, status: abort_init\n",
						 (char *)&(addrinfo_in.address),
						 addrinfo_in.port);
					close(socket_in);
					return 1;
				}
				packlen_inbound =
				    packlen_inbound + packlen_inbound_append;
			}
	...
	case 0x02:
		break;
	default:
		if ((inbound[packlen_inbound - 1] == 1)
		    || (inbound[packlen_inbound - 1] == 2)) {
			packlen_inbound_append =
			    packlen_inbound + recv(socket_in,
						   inbound + packlen_inbound,
						   BUFSIZ - packlen_inbound, 0);
			if (packlen_inbound_append == 0) {
				mksysmsg(0, logfile, runmode,
					 conf_in->log.level, 1,
					 "src: %s:%d, status: abort_init\n",
	...
	if (inbound[0] == 0xFE) {
		...
		if (motd_version == PVER_LEGACYM3) {
			p_motd_legacy inbound_info =
			    packet_read_legacy_motd(inbound);
			conf_proxy proxyinfo =
			    config_proxy_search(conf_in, inbound_info.address);
			if (proxyinfo.valid == 0) {
				...
				packlen_rewrited =
				    make_motd_legacy(rewrited,
						     "[Proxy] Use a legit address to play!",
						     motd_version,
						     inbound_info.version);
			...
			if (proxyinfo.srvenabled == 1) {
				...
				if (net_srvresolve
				    (proxyinfo.address, srvrecords) > 0) {
					proxyinfo.address =
					    (char *)realloc(proxyinfo.address,
							    strlen(srvrecords
								   [0].target) +
							    1);
					strcpy(proxyinfo.address,
					       srvrecords[0].target);
			...
			*socket_out =
			    net_socket(NETSOCK_CONN, connaddr.family,
				       &(connaddr.addr), proxyinfo.port, 0);
			if (*socket_out == -1) {
			...
			case 0:
				mksysmsg(0, logfile, runmode,
					 conf_in->log.level, outmsg_level,
					 "src: %s:%d, type: motd, vhost: %s, dst: %s:%d, status: accept\n",
					 (char *)&(addrinfo_in.address),
					 addrinfo_in.port, inbound_info.address,
					 proxyinfo.address, proxyinfo.port);
				if (proxyinfo.pheader == 1) {
					if (connaddr.family ==
					    addrinfo_in.family) {
						net_addrp addrinfo_out =
						    net_ntop(connaddr.family,
							     &(connaddr.addr),
							     0);
						if (addrinfo_in.family ==
						    AF_INET) {
							packlen_pheader =
							    sprintf(pheader,
								    "PROXY TCP4 %s %s %d %d\r\n",
				...
				if (proxyinfo.rewrite == 1) {
					inbound_info.address =
					    realloc(inbound_info.address,
						    strlen(proxyinfo.address) +
						    1);
					strcpy(inbound_info.address,
					       proxyinfo.address);
					inbound_info.port = proxyinfo.port;
					packlen_rewrited =
					    packet_write_legacy_motd(rewrited,
								     inbound_info);
					send(*socket_out, rewrited,
					     packlen_rewrited, 0);
				} else {
					send(*socket_out, inbound,
					     packlen_inbound, 0);
				}
			...
			case NET_ECONNECT:
				...
				packlen_rewrited =
				    make_motd_legacy(rewrited,
						     "[Proxy] Server Temporarily Unavailable.",
						     motd_version,
						     inbound_info.version);
				send(socket_in, rewrited, packlen_rewrited, 0);
				close(socket_in);
			...
			packlen_rewrited =
			    make_motd_legacy(rewrited,
					     "Proxy: Please use direct connect.",
					     protocol_identify(inbound), 0);
			send(socket_in, rewrited, packlen_rewrited, 0);
			close(socket_in);
			return 5;
		}
		...
		if (login_version == PVER_LEGACYL1) {
			mksysmsg(0, logfile, runmode, conf_in->log.level, 1,
				 ...
				 addrinfo_in.port);
			packlen_rewrited =
			    make_kickreason_legacy(rewrited,
						   "Proxy: Unsupported client, use 12w04a or later!");
			send(socket_in, rewrited, packlen_rewrited, 0);
			close(socket_in);
			return 5;
			...
			packlen_rewrited =
			    make_kickreason_legacy(rewrited,
						   "Proxy: Unsupported client, use 12w18a or later!");
			send(socket_in, rewrited, packlen_rewrited, 0);
			close(socket_in);
			return 5;
		} else if ((login_version == PVER_LEGACYL2)
			   || (login_version == PVER_LEGACYL4)) {
			p_login_legacy inbound_info =
			    packet_read_legacy_login(inbound, packlen_inbound,
						     login_version);
			conf_proxy proxyinfo =
			    config_proxy_search(conf_in, inbound_info.address);
			if (proxyinfo.valid == 0) {
				...
				packlen_rewrited =
				    make_kickreason_legacy(rewrited,
							   "Proxy: Please use a legit name to connect!");
				send(socket_in, rewrited, packlen_rewrited, 0);
				close(socket_in);
				return 3;
			}
			...
			*socket_out =
			...
			case 0:
				...
				if (proxyinfo.rewrite == 1) {
					...
					inbound_info.port = proxyinfo.port;
					packlen_rewrited =
					    packet_write_legacy_login
					    (inbound_info, rewrited);
					send(*socket_out, rewrited,
					     packlen_rewrited, 0);
				} else {
					send(*socket_out, inbound,
			...
			case NET_ECONNECT:
				if (mkoutbound_status == NET_ENORECORD) {
					...
					packlen_rewrited =
					    make_kickreason_legacy(rewrited,
								   "Proxy(Internal): Temporarily failed to resolve the address for the target server, please try again later.");
				} else if (mkoutbound_status == NET_ECONNECT) {
					mksysmsg(0, logfile, runmode,
						 conf_in->log.level,
						 outmsg_level,
					...
					packlen_rewrited =
					    make_kickreason_legacy(rewrited,
								   "Proxy(Internal): Failed to connect to the target server, please try again later.");
				}
				send(socket_in, rewrited, packlen_rewrited, 0);
				close(socket_in);
				config_proxy_search_destroy(&proxyinfo);
		...
		if (inbound_info.version == 0) {
			if (inbound[inbound[0]] == 1) {
				...
				packlen_rewrited =
				    make_motd(rewrited,
					      "[Proxy] Use 13w42a or later to play!",
					      inbound_info.version);
				...
				mksysmsg(0, logfile, runmode,
					 ...
					 addrinfo_in.port);
				packlen_rewrited =
				    make_kickreason(rewrited,
						    "Proxy: Unsupported client, use 13w42a or later!");
			}
			send(socket_in, rewrited, packlen_rewrited, 0);
		...
		mkoutbound_status = 0;
		net_addr connaddr =
		    net_resolve_dual(proxyinfo.address, addrinfo_in.family,
				     netpriority_enabled);
		if (connaddr.family == 0) {
			mkoutbound_status = NET_ENORECORD;
		}
		*socket_out =
		    net_socket(NETSOCK_CONN, connaddr.family, &(connaddr.addr),
			       proxyinfo.port, 0);
		if (*socket_out == -1) {
			mkoutbound_status = NET_ECONNECT;
		}
		...
		case 0:
			...
			if (proxyinfo.rewrite == 1) {
				inbound_info.address =
				    realloc(inbound_info.address,
					    strlen(proxyinfo.address) + 1);
				strcpy(inbound_info.address, proxyinfo.address);
				inbound_info.port = proxyinfo.port;
				packlen_rewrited =
				    packet_write(rewrited, inbound_info);
				send(*socket_out, rewrited, packlen_rewrited,
				     0);
			} else {
