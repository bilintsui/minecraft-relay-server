...
`LISTENER_HOSTS_FILENAME` defaults to the C string `"/etc/hosts"` and can select another local-static hosts input at compilation. String-valued overrides must retain valid C string quoting. The listener loads the snapshot at startup and on configuration reload, and also monitors the selected path with inotify. Direct writes, atomic replacement, deletion/recreation, symbolic-link and hard-link publication, linked-target updates, watch invalidation and queue overflow are coalesced into atomic route-generation replacement; a pending replacement waits until the prior retired generation is no longer pinned. Creation of the watched path may trigger a reload before a regular-file writer closes, followed by another reload on close; prepare a complete file and rename it into place when an atomic update is required. Monitor setup or refresh failure is non-fatal, is logged, and leaves `SIGUSR1` configuration reload as the fallback. This mechanism does not enable NSS or search-domain lookup.
